...
def train(model, train_loader, eval_loader, args, qid2type):
    ...
    for epoch in range(num_epochs):
        ...
        # for i, (v, s, q, a, b) in tqdm(enumerate(train_loader), ncols=100,
        #                                                                           desc="Epoch %d" % (epoch + 1),
        #                                                                           total=len(train_loader)):
            total_step += 1
            ...
            if mode == 'updn':
                # loss_q, loss_v = model(v, s, q, a, b, None, None, 'first')
                pred, loss, _ = model(v, s, q, a, b, None)
                ...
                ## first train
                pred, loss, word_emb = model(v, s, q, a, b, None)
                ...
                false_ans = torch.ones(64, 2274).cuda()
                ...
                pred, loss, _ = model(v, s, q3, a2, b, None)
                ...
                if random_num <= qvp:
                    ...
                    pred, loss, _ = model(v, s, q, a2, b, v_mask)
...
def evaluate(model, dataloader, qid2type):
    ...
    for v, s, q, a, b, qids, _ in tqdm(dataloader, ncols=100, total=len(dataloader), desc="eval"):
        ...
        # pred, _, _ = model(v, s, q, None, None, None, None, 'second')
